...
use crate::types::{PortStatus, ScanConfig};
...
#[command(
    name = "rmap",
    version,
    about = "High-performance network scanner written in Rust",
    long_about = "Supports ICMP host discovery and TCP port scanning.\n\
                  Requires elevated privileges (root/sudo on Linux, \
                  Administrator on Windows) for ICMP."
)]
struct Args {
    /// Targets: single IP, CIDR (192.168.1.0/24), or dash range (192.168.1.1-254)
    #[arg(required = true)]
    targets: Vec<String>,

    /// Ports to scan: comma-separated or ranges (e.g. 22,80,443,1-1024).
    /// Use "top1000" for nmap-compatible top-1000 list.
    #[arg(short, long, default_value = "top1000")]
    ports: String,

    /// Only perform ICMP host discovery, skip port scan
    #[arg(long)]
    icmp_only: bool,

    /// Skip ICMP, assume all targets are alive and scan ports directly
    #[arg(long)]
    port_only: bool,
...
fn parse_targets(raw: &[String]) -> Result<Vec<IpAddr>> {
    let mut ips = Vec::new();
    ...
    for s in raw {
        ...
        // Dash range: full (10.0.0.1-10.0.0.20) or short-form (10.0.0.1-20)
        if s.contains('-') {
            let (start_str, end_str) = s.split_once('-').unwrap();
            let start: IpAddr = start_str.parse()?;

            let end: IpAddr = if end_str.contains('.') || end_str.contains(':') {
                end_str.parse()?
            } else {
                // Short form: replace last octet
                match start {
                    IpAddr::V4(v4) => {
                        let end_oct: u8 = end_str.parse()?;
                        let [a, b, c, _] = v4.octets();
                        IpAddr::V4(std::net::Ipv4Addr::new(a, b, c, end_oct))
                    }
                    IpAddr::V6(_) => bail!("Short-form ranges are not supported for IPv6"),
                }
            };

            let (s_u, e_u) = match (start, end) {
                (IpAddr::V4(s), IpAddr::V4(e)) => (u32::from(s), u32::from(e)),
                _ => bail!("Mixed IP versions in range: {}", s),
            };
            if s_u > e_u {
                bail!("Range start is after end: {}", s);
            }
            for u in s_u..=e_u {
                ips.push(IpAddr::V4(std::net::Ipv4Addr::from(u)));
            }
            continue;
        }
...
fn print_plain(results: &[crate::types::HostResult], _open_only: bool) {
    for host in results {
        let alive_tag = match host.alive {
            Some(true) => "up",
            Some(false) => "down",
            None => "unknown",
        };

        let open_count = host.ports.iter().filter(|p| p.status == PortStatus::Open).count();
        println!(
            "\nHost: {}  status={}  open_ports={}",
            host.ip, alive_tag, open_count
        );
...
async fn main() -> Result<()> {
    ...
    if args.icmp_only && args.port_only {
        bail!("--icmp-only and --port-only are mutually exclusive");
    }

    let targets = parse_targets(&args.targets)?;
    if targets.is_empty() {
        bail!("No valid targets parsed");
    }

    let ports = if args.icmp_only {
        vec![]
    } else {
    ...
    eprintln!(
        "rmap  targets={}  ports={}  concurrency={}  timeout={}ms",
        targets.len(),
        ports.len(),
        args.concurrency,
        args.timeout,
    ...
    if args.output == "json" {
        ...
        let open_total: usize = results
            ...
            .sum();
        eprintln!(
            "\nScan complete: {} host(s) up, {} open port(s) found.",
            up, open_total
        );
    }
...
// ---------------------------------------------------------------------------
// Top-1000 port list (nmap-compatible)
// ---------------------------------------------------------------------------

fn top1000_ports() -> Vec<u16> {
    let mut ports: Vec<u16> = vec![
        ...
        256, 259, 264, 280, 301, 306, 311, 340, 366, 389, 406, 407, 416, 417, 425, 427,
        443, 444, 445, 458, 464, 465, 481, 497, 500, 512, 513, 514, 515, 524, 541, 543,
        544, 545, 548, 554, 555, 563, 587, 593, 616, 617, 625, 631, 636, 646, 648, 666,
        667, 668, 683, 687, 691, 700, 705, 711, 714, 720, 722, 726, 749, 765, 777, 783,
        787, 800, 801, 808, 843, 873, 880, 888, 898, 900, 901, 902, 903, 911, 912, 981,
        987, 990, 992, 993, 995, 999, 1000, 1001, 1002, 1007, 1009, 1010, 1011, 1021,
        ...
        1100, 1102, 1104, 1105, 1106, 1107, 1108, 1110, 1111, 1112, 1113, 1114, 1117,
        1119, 1121, 1122, 1123, 1124, 1126, 1130, 1131, 1132, 1137, 1138, 1141, 1145,
        1147, 1148, 1149, 1151, 1152, 1154, 1163, 1164, 1165, 1166, 1169, 1174, 1175,
        1183, 1185, 1186, 1187, 1192, 1198, 1199, 1201, 1213, 1216, 1217, 1218, 1233,
        1234, 1236, 1244, 1247, 1248, 1259, 1271, 1272, 1277, 1287, 1296, 1300, 1301,
        1309, 1310, 1311, 1322, 1328, 1334, 1352, 1417, 1433, 1434, 1443, 1455, 1461,
        ...
        8654, 8701, 8800, 8873, 8888, 8899, 8994, 9000, 9001, 9002, 9003, 9009, 9010,
        9011, 9040, 9050, 9071, 9080, 9081, 9090, 9091, 9099, 9100, 9101, 9102, 9103,
        9110, 9111, 9200, 9207, 9220, 9290, 9415, 9418, 9485, 9500, 9502, 9503, 9535,
        9575, 9593, 9594, 9595, 9618, 9666, 9876, 9877, 9878, 9898, 9900, 9917, 9929,
        9943, 9944, 9968, 9998, 9999, 10000, 10001, 10002, 10003, 10004, 10009, 10010,
        10012, 10024, 10025, 10082, 10180, 10215, 10243, 10566, 10616, 10617, 10621,
        10626, 10628, 10629, 10778, 11110, 11111, 11967, 12000, 12174, 12265, 12345,
        13456, 13722, 13782, 13783, 14000, 14238, 14441, 14442, 15000, 15002, 15003,
        ...
        19801, 19842, 20000, 20005, 20031, 20221, 20222, 20828, 21571, 22939, 23502,
        24444, 24800, 25734, 25735, 26214, 27000, 27352, 27353, 27355, 27356, 27715,
        28201, 30000, 30718, 30951, 31038, 31337, 32768, 32769, 32770, 32771, 32772,
        32773, 32774, 32775, 32776, 32777, 32778, 32779, 32780, 32781, 32782, 32783,
        32784, 32785, 33354, 33899, 34571, 34572, 34573, 35500, 38292, 40193, 40911,
        41511, 42510, 44176, 44442, 44443, 44501, 45100, 48080, 49152, 49153, 49154,
        49155, 49156, 49157, 49158, 49159, 49160, 49161, 49163, 49165, 49167, 49175,
        49176, 49400, 49999, 50000, 50001, 50002, 50003, 50006, 50300, 50389, 50500,
